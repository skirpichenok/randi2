...
public class Step5 {
	...
	public static enum AlgorithmPanelId {
		COMPLETE_RANDOMIZATION("completeRandomization"), BIASEDCOIN_RANDOMIZATION("biasedCoinRandomization"), BLOCK_RANDOMIZATION("blockRandomization"), TRUNCATED_RANDOMIZATION("truncatedRandomization"), URN_MODEL("urnModel"), MINIMIZATION("minimization"), RESPONSE_ADAPTIVE_RANDOMIZATION("responseAdaptiveRandomization");
	...
	public List<SelectItem> getAlgorithms() {
		if (algorithms == null) {
			...
			algorithms.add(new SelectItem(AlgorithmPanelId.RESPONSE_ADAPTIVE_RANDOMIZATION.toString(), bundle.getString(ResponseAdaptiveRConfig.class.getCanonicalName() + ".short")));
		}
		return algorithms;
	}
	...
	public void setSelectedAlgorithmPanelId(String selectedAlgorithmPanelId) {
		...
		if (selectedAlgorithmPanelId.equals(AlgorithmPanelId.BLOCK_RANDOMIZATION.toString()) && !BlockRandomizationConfig.class.isInstance(trialHandler.getCurrentObject().getRandomizationConfiguration())) {
			trialHandler.getCurrentObject().setRandomizationConfiguration(new BlockRandomizationConfig());
		} else if (selectedAlgorithmPanelId.equals(AlgorithmPanelId.URN_MODEL.toString()) && !UrnDesignConfig.class.isInstance(trialHandler.getCurrentObject().getRandomizationConfiguration())) {
			trialHandler.getCurrentObject().setRandomizationConfiguration(new UrnDesignConfig());
		} else if (selectedAlgorithmPanelId.equals(AlgorithmPanelId.MINIMIZATION.toString()) && !MinimizationConfig.class.isInstance(trialHandler.getCurrentObject().getRandomizationConfiguration())) {
			trialHandler.getCurrentObject().setRandomizationConfiguration(new MinimizationConfig());
		} else if (selectedAlgorithmPanelId.equals(AlgorithmPanelId.RESPONSE_ADAPTIVE_RANDOMIZATION.toString()) && !ResponseAdaptiveRConfig.class.isInstance(trialHandler.getCurrentObject().getRandomizationConfiguration())) {
			trialHandler.getCurrentObject().setRandomizationConfiguration(new ResponseAdaptiveRConfig());
		}
	}
